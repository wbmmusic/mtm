...
export const Sequence: React.FC = () => {
  ...
  // Sequence deletion confirmation modal
  const [confirmDeleteSequenceModal, setConfirmDeleteSequenceModal] =
    useState<ConfirmDeleteSequenceModalState>(
      defaultConfirmDeleteSequenceModal
    );

  // Timeline clearing confirmation modal
  const [showClearTimelineConfirm, setShowClearTimelineConfirm] =
    useState(false);

  // Position editing modal (create/edit positions)
  const [positionModal, setPositionModal] =
    useState<PositionModalState>(defaultPositionModal);

  // Position deletion confirmation modal
  const [deletePositionModal, setDeletePositionModal] =
    useState<DeletePositionModalState>(defaultDeletePositionModal);

  // Position selection modal (choose existing position)
  const [selectPositionModal, setSelectPositionModal] =
    useState<SelectPositionModalState>(defaultSelectPositionModal);
  ...
  const [timelineObjects, setTimelineObjects] = useState<TimelineItem[] | null>(
    null
  );
  ...
  const [modalPositions, setModalPositions] = useState<PositionType[] | null>(
    null
  );
  ...
  const loadSequence = useCallback(() => {
    getSequence(robotPath as string, sequenceId as string)
      .then((res: SequenceType) => {
        ...
        tempSeq.actions = (tempSeq.actions || []).map(
          act => ({ ...(act as Action), id: uuid() }) as Action
        );
  ...
  const haveEverything = () => {
    ...
    for (const act of acts) {
      ...
      if (act.appId) {
        const idx =
          timelineObjects?.findIndex(obj => obj.appId === act.appId) ?? -1;
  ...
  const DelayItem: React.FC<{ itm: TimelineItem }> = ({ itm }) => (
    <Stack height={"100%"} spacing={0.5}>
      <Box textAlign="center">
        <PixelText>{itm.content}</PixelText>
      </Box>
      <Box
        height={"100%"}
        display="flex"
  ...
  const onDragEnd = (res: DropResult) => {
    ...
    if (
      ...
      setSequence((old: SequenceType | null) =>
        old ? { ...old, actions: actionsCopy } : old
      );
    } else if (
      ...
      setSequence((old: SequenceType | null) =>
        old ? { ...old, actions: actionsCpy } : old
      );
    }
  };
  ...
  const makeItem = (itm: TimelineItem) => {
    if (itm.type === "delay")
      return <DelayItem itm={itm} />; // Fixed duration waits
    else if (itm.type === "move")
      return <PositionItem itm={itm} />; // Servo movements
  ...
  const handlePositionModal = (
    type: "cancel" | "createPosition" | "updatePosition",
          ...
          <Box sx={{ paddingLeft: "4px" }}>
            <Stack direction="row" spacing={2} alignItems="center">
              ...
              {(sequence?.actions?.length || 0) > 0 ? (
                <DangerButton
                  size="small"
                  onClick={() => setShowClearTimelineConfirm(true)}
                >
                  Clear Timeline
                </DangerButton>
              ) : null}
          ...
          <Droppable droppableId="timeline" direction="horizontal">
            {provided => (
              <Stack
                ...
                sx={droppableStyle}
              >
                {((sequence?.actions || []) as Action[]).map(
                  (a, idx: number) => {
                    const found = a.appId
                      ? (timelineObjects?.find(
                          (obj: TimelineItem) => obj.appId === a.appId
                        ) ?? null)
                      : null;
  ...
  const makeActionsFromRefs = (): Action[] => {
    ...
    for (const act of (sequence?.actions || []) as Action[]) {
      ...
      if (!theObjRaw) {
        ...
        console.debug(
          "Sequence.makeActionsFromRefs: missing referenced position with appId",
          act.appId,
          "- this position may have been deleted"
        );
      ...
      if (theObj.type === "move" && lastType === "move") {
        ...
        previouState.content =
          (previouState.content || "") + "/" + (theObj.content || "");
      ...
      <HeaderBar>
        <Stack direction="row" width={"100%"} spacing={1} alignItems="center">
          <RetroTextField
            ...
            value={sequence.name}
            onChange={e =>
              setSequence((old: any) => ({ ...old, name: e.target.value }))
            }
          />
          <Box>
            ...
            <Tooltip title="Edit Position">
              <IconButton
                ...
                onClick={() => {
                  ...
                  setSelectPositionModal({ show: true });
                }}
              >
                <EditIcon />
              </IconButton>
