...
function requireDist() {
  ...
  dist.getLineLatest = dist.newDevice = dist.newLine = dist.install = dist.getCurrent = dist.setBase = dist.downloadFirmware = dist.getLatest = dist.getFirmwares = dist.getDevices = dist.getLines = dist.getLine = dist.wbmVer = void 0;
  const axios_12 = __importDefault(/* @__PURE__ */ requireAxios());
  const node_events_1 = require$$1$3;
  const wbmVer = new node_events_1.EventEmitter();
  dist.wbmVer = wbmVer;
  const setBase = (base) => axios_12.default.defaults.baseURL = base;
  dist.setBase = setBase;
  const downloadFirmware = (id, outputLocationPath) => __awaiter(void 0, void 0, void 0, function* () {
    const { createWriteStream, stat, unlinkSync, unwatchFile, watchFile } = yield Promise.resolve().then(() => __importStar(require$$2));
    const writer = createWriteStream(outputLocationPath);
    return new Promise((resolve, reject) => {
      let fileSize = 0;
      watchFile(outputLocationPath, function() {
        stat(outputLocationPath, function(_err, stats) {
          if (stats.size === void 0 || typeof stats.size !== "number")
            return;
          let percent = Math.floor(100 * stats.size / fileSize);
          wbmVer.emit("downloadStatus", percent);
        });
      });
      axios_12.default.get("/download", { params: { id }, responseType: "stream" }).then((response) => {
        ...
        writer.on("error", (err) => {
          error = err;
          writer.close();
          unwatchFile(outputLocationPath);
          unlinkSync(outputLocationPath);
          reject(err);
        });
        writer.on("close", () => {
          if (!error) {
            wbmVer.emit("downloadStatus", "complete");
            unwatchFile(outputLocationPath);
            resolve(true);
          }
        });
  ...
  dist.downloadFirmware = downloadFirmware;
  const getLatest = (lineID, deviceID) => __awaiter(void 0, void 0, void 0, function* () {
    return new Promise((resolve, reject) => {
      axios_12.default.get("/device", { params: { lineID, deviceID } }).then((res) => {
        let device = res.data;
        let current = device.current;
        let firm = device.firmware.find((fw) => fw.version === current);
        firm.id;
        firm.name;
        resolve(firm);
  ...
  dist.getLatest = getLatest;
  const getLines = () => __awaiter(void 0, void 0, void 0, function* () {
    return new Promise((resolve, reject) => {
      let lines = [];
      axios_12.default.get("/lines").then((res) => {
        res.data.lines.forEach((line) => {
          let tempLine = Object.assign({}, line);
          delete tempLine.devices;
          lines.push(tempLine);
        });
        resolve(lines);
      }).catch((err) => reject(err));
  ...
  dist.getLines = getLines;
  const getLine = (lineID) => __awaiter(void 0, void 0, void 0, function* () {
    return new Promise((resolve, reject) => {
      axios_12.default.get("/line", { params: { lineID } }).then((res) => resolve(res.data)).catch((err) => reject(err));
    });
  });
  dist.getLine = getLine;
  const getDevices = (lineID) => __awaiter(void 0, void 0, void 0, function* () {
    return new Promise((resolve, reject) => {
      axios_12.default.get("/devices", { params: { lineID } }).then((res) => {
        resolve(res.data);
      }).catch((err) => reject(err));
    });
  });
  dist.getDevices = getDevices;
  const getFirmwares = (lineID, deviceID) => __awaiter(void 0, void 0, void 0, function* () {
    return new Promise((resolve, reject) => {
      axios_12.default.get("/firmwares", { params: { lineID, deviceID } }).then((res) => {
        resolve(res.data);
      }).catch((err) => reject(err));
    });
  });
  dist.getFirmwares = getFirmwares;
  const getCurrent = (productLine, device) => __awaiter(void 0, void 0, void 0, function* () {
    return new Promise((resolve, reject) => __awaiter(void 0, void 0, void 0, function* () {
      try {
        let lines = yield getLines();
        if (!lines)
          return reject(new Error("Error getting lines"));
        const theLine = lines.find((ln) => ln.path === productLine);
        if (!theLine)
          return reject(new Error("Didn't find that line"));
        let lineID = theLine.id;
        if (lineID === void 0)
          reject(new Error("Error getting lineID"));
        let line = yield getLine(lineID);
        if (!line)
          return reject(new Error("Error getting line"));
        if (!line.devices)
          return reject(new Error("Error getting line"));
        const theDevice = line.devices.find((dev) => dev.path === device);
        if (!theDevice)
          return reject(new Error("Error getting the Device"));
        let deviceID = theDevice.id;
        if (deviceID === void 0)
          reject(new Error("Error getting Device ID"));
        let latest = yield getLatest(lineID, deviceID);
        if (latest === void 0)
          reject(new Error("Error getting latest"));
        resolve(latest);
      } catch (error) {
        reject(error);
      }
  ...
  dist.getCurrent = getCurrent;
  const install = (latestVersion, pathToInstallers) => {
    const { chmodSync } = require$$6;
    const { spawn } = require$$4$2;
    try {
      chmodSync(require("path").join(pathToInstallers, latestVersion.name), "0744");
      let inst = spawn("./" + latestVersion.name, { detached: true, stdio: ["ignore", "ignore", "ignore"], cwd: pathToInstallers }).unref();
    } catch (error) {
      throw error;
    }
  };
  dist.install = install;
  const newLine = (name) => __awaiter(void 0, void 0, void 0, function* () {
    return new Promise((resolve, reject) => {
      axios_12.default.post("/lines", { name }).then((res) => resolve(res.data)).catch((err) => reject(err));
    });
  });
  dist.newLine = newLine;
  const newDevice = (line, name) => __awaiter(void 0, void 0, void 0, function* () {
    return new Promise((resolve, reject) => {
      axios_12.default.post("/device", { line, name }).then((res) => resolve(res.data)).catch((err) => reject(err));
    });
  });
  dist.newDevice = newDevice;
  const getLineLatest = (linePath) => __awaiter(void 0, void 0, void 0, function* () {
    return new Promise((resolve, reject) => {
      axios_12.default.get("/linelatest", { data: { linePath } }).then((res) => resolve(res.data)).catch((err) => reject(err));
    });
  });
  dist.getLineLatest = getLineLatest;
  return dist;
}
...
const makeWaitData = (type2, val) => {
  let commandByte = commandIds.time << 6 | 1 << 5 | type2 << 4 | val & 15;
  return [commandByte];
};
const pathToUserData = node_path.join(electron.app.getPath("userData"), "data");
const pathToUserSettings = node_path.join(pathToUserData, "settings.json");
const pathToRobots = node_path.join(pathToUserData, "robots");
const pathToFirmwareFolder = node_path.join(pathToUserData, "firmware");
const readSettings = () => {
  return JSON.parse(require$$2.readFileSync(pathToUserSettings, "utf8"));
};
const saveSettings = () => {
  require$$2.writeFileSync(pathToUserSettings, JSON.stringify(global.settings, null, " "));
...
const checkFolders = () => {
  ...
  if (!require$$2.existsSync(pathToUserData)) require$$2.mkdirSync(pathToUserData);
  if (!require$$2.existsSync(pathToRobots)) require$$2.mkdirSync(pathToRobots);
  if (!require$$2.existsSync(pathToFirmwareFolder)) require$$2.mkdirSync(pathToFirmwareFolder);
  if (!require$$2.existsSync(pathToUserSettings)) {
...
const getRobots = () => {
  const folders = require$$2.readdirSync(pathToRobots, { withFileTypes: true }).filter((dirent) => dirent.isDirectory());
  let robots = [];
  folders.forEach((folder) => {
    const pathToRobot = node_path.join(pathToRobots, folder.name);
    const pathToRobotFile = node_path.join(pathToRobot, "robot.json");
    if (require$$2.existsSync(pathToRobot) && require$$2.existsSync(pathToRobotFile)) {
      robots.push(JSON.parse(require$$2.readFileSync(pathToRobotFile, "utf8")));
    }
  });
  return robots;
};
    ...
    if (idx === 0) {
      ...
      if (out[prevIdx].type === act.type) {
        if (act.type === "delay") {
          ...
          act.servos?.forEach((srv, idx2) => {
            if (srv.enabled) {
              if (!out[prevIdx].servos) out[prevIdx].servos = [];
              if (!out[prevIdx].servos[idx2]) out[prevIdx].servos[idx2] = { enabled: false, value: 0 };
              out[prevIdx].servos[idx2].value = srv.value;
              out[prevIdx].servos[idx2].enabled = true;
            }
          });
...
distExports.setBase("http://versions.wbmtek.com/api");
const compareToLatest = () => {
  const win2 = getWin();
  if (global.connectedDeviceInfo && win2) {
    if (!global.connectedDeviceInfo.model) {
      console.log("Connected device missing model");
      return;
    }
    const pathToDeviceFolder = node_path.join(pathToFirmwareFolder, global.connectedDeviceInfo.model.toLowerCase());
    const pathToLatestData = node_path.join(pathToDeviceFolder, "latest.json");
    if (require$$2.existsSync(pathToDeviceFolder)) {
      let latest = JSON.parse(require$$2.readFileSync(pathToLatestData, "utf8"));
      ...
      if (latest.version > currentFirmware) {
        ...
        win2.webContents.send("firmwareAvailable", latest);
      } else {
...
const checkForFirmwareUpdates = async () => {
  try {
    const lines = await distExports.getLines();
    const mtmLine = lines.find((line) => line.name === "MTM");
    if (!mtmLine) {
      throw new Error("MTM line not found");
    }
    const lineID = mtmLine.id;
    const devices = await distExports.getDevices(lineID);
    console.log(pathToFirmwareFolder);
    devices.forEach((device) => {
      const pathToDeviceFolder = node_path.join(pathToFirmwareFolder, device.path);
      if (!require$$2.existsSync(pathToDeviceFolder)) require$$2.mkdirSync(pathToDeviceFolder);
    });
    for (let i = 0; i < devices.length; i++) {
      const pathToDeviceFolder = node_path.join(pathToFirmwareFolder, devices[i].path);
      const latest = await distExports.getLatest(lineID, devices[i].id);
      if (latest) {
        if (!require$$2.readdirSync(pathToDeviceFolder).includes(latest.name)) {
          const folderContents = require$$2.readdirSync(pathToDeviceFolder);
          folderContents.forEach((file) => require$$2.rmSync(node_path.join(pathToDeviceFolder, file)));
          await distExports.downloadFirmware(latest.id, node_path.join(pathToDeviceFolder, latest.name));
          require$$2.writeFileSync(node_path.join(pathToDeviceFolder, "latest.json"), JSON.stringify(latest, null, "  "));
          compareToLatest();
        }
      }
    }
...
process.setMaxListeners(1e9);
const bootEmitter = new require$$1$3.EventEmitter();
const usbTarget = [
  { vid: 9025, pid: 67 },
  { vid: 1003, pid: 9220 }
];
...
const getPorts = async () => {
  try {
    const rawPorts = await serialport.SerialPort.list();
    const filtered = rawPorts.filter((prt) => prt.path && !prt.path.includes("BLTH") && !prt.path.includes("Bluetooth"));
    const mapped = filtered.map((prt) => {
      const pi = {
        path: String(prt.path),
        vendorId: prt.vendorId ? String(prt.vendorId) : void 0,
        productId: prt.productId ? String(prt.productId) : void 0,
        serialNumber: prt.serialNumber ? String(prt.serialNumber) : void 0,
        friendlyName: prt.friendlyName ? String(prt.friendlyName) : prt.manufacturer ? String(prt.manufacturer) : void 0,
        manufacturer: prt.manufacturer ? String(prt.manufacturer) : void 0
      };
      return pi;
    });
    return mapped;
...
const sendStream = (data) => {
  ...
  port.write(out, (err) => {
    if (err) console.error("sendStream error", err);
  });
};
const usbStatus = () => {
  const win2 = getWin();
  if (port && win2) {
    win2.webContents.send("usb_status", true);
    return true;
  } else {
    win2?.webContents.send("usb_status", false);
    return false;
  }
};
...
const openPort = async () => {
  ...
  try {
    ...
    if (thePorts.length > 0) {
      ...
      port = new serialport.SerialPort({ path: thePorts[0].path, baudRate: 9600 });
      return new Promise((resolve, reject) => {
        port.on("open", async () => {
  ...
  try {
    ...
    setTimeout(async () => {
      try {
        ...
        console.log("Error on 2nd attempt to connect", error2);
      }
    }, 1e3);
  }
};
const sendProgramCommand = async () => {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => exit(new Error("sendProgramCommand timed out")), 1e3);
    port.on("data", handleData);
    port.write("WBM:LOAD");
  });
};
const sendHalfPage = async (pageHalf, half) => {
  return new Promise((resolve, reject) => {
    ...
    let timer = setTimeout(() => exit(new Error("sendPage timed out")), 1e3);
    port.on("data", handleData);
    if (half === 0) {
...
const sendDone = async () => {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => exit(new Error("sendDone timed out")), 1e3);
    port.on("data", handleData);
...
const sendPages = async (pages) => {
  const win2 = getWin();
  let pagesSent = 0;
  win2?.webContents.send("upload_progress", { show: true, value: 0 });
  for (const thePage of pages) {
    await sendPage(thePage);
    console.log("Sent Page", pagesSent);
    pagesSent++;
    win2?.webContents.send("upload_progress", { show: true, value: 100 * pagesSent / pages.length });
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const handleData = (data) => {
      if (!data.toString().includes("WBM:FLASHINFO")) {
        exit(void 0, new Error(`Unexpected data in getDeviceInfo ${data.toString()}`));
        return;
      }
      if (data.length < 6) {
        exit(void 0, new Error("getDeviceInfo: response too short"));
        return;
      }
      ...
      try {
        ...
        exit(void 0, new Error("getDeviceInfo: failed to parse trailer"));
      }
    };
    const timer = setTimeout(() => exit(void 0, new Error("getDeviceInfo timed out")), 1e3);
    port.on("data", handleData);
...
const handleFirmwareUpload = async (file) => {
  ...
  if (!global.connectedDeviceInfo.serialNumber.includes("BOOT:")) {
    if (global.connectedDeviceInfo.serialNumber.includes("WBM:")) {
      ...
      win2?.webContents.send("upload_progress", { show: true, value: null });
      await sendBootToBootloader();
      await upload(fileArray);
      win2?.webContents.send("upload_progress", { show: false, value: null });
...
const uploadFirmware = async () => {
  ...
  const devLatest = JSON.parse(require$$2.readFileSync(node_path.join(pathToDeviceFolder, "latest.json"), "utf8"));
  const pathToLatestFirmwareFile = node_path.join(pathToDeviceFolder, devLatest.name);
  const file = require$$2.readFileSync(pathToLatestFirmwareFile);
...
electron.ipcMain.on("uploadFirmware", () => uploadFirmware());
electron.ipcMain.on("upload", async (e, actions) => {
...
electron.ipcMain.on("play", (e, file) => {
  const win2 = getWin();
  if (global.settings?.sound && win2) {
    win2.webContents.send("play_file", file);
  }
});
electron.ipcMain.on("get_usb_status", () => {
  usbStatus();
});
electron.ipcMain.handle("sound", (e, onOff) => {
  if (global.settings) {
...
electron.ipcMain.handle("sendValue", async (e, data) => {
  console.log("Send Serial", data);
  sendStream(data);
  return true;
});
electron.ipcMain.handle("getRobots", async () => {
  return getRobots();
});
electron.ipcMain.handle("getRobot", async (e, path) => {
  const pathToRobot = node_path.join(pathToRobots, path, "robot.json");
  if (!require$$2.existsSync(pathToRobot)) throw new Error("Path to robot" + path + " does not exist");
  const bot = JSON.parse(require$$2.readFileSync(pathToRobot, "utf8"));
  return bot;
});
electron.ipcMain.handle("deleteRobot", async (e, path) => {
  console.log("Delete Robot", path);
  const robotPath = node_path.join(pathToRobots, path);
  if (require$$2.existsSync(robotPath)) require$$2.rmdirSync(robotPath, { recursive: true });
  else throw new Error("Folder with path " + path + " does not exist");
  return getRobots();
});
electron.ipcMain.handle("saveRobot", async (e, robot) => {
  console.log("Save Robot", robot.name);
  const robotPath = node_path.join(pathToRobots, robot.path);
  const robotFilePath = node_path.join(robotPath, "robot.json");
  if (require$$2.existsSync(robotPath)) {
    throw new Error("Robot folder path already exists");
  }
  require$$2.mkdirSync(robotPath);
  require$$2.writeFileSync(robotFilePath, JSON.stringify(robot, null, " "));
  return void 0;
});
electron.ipcMain.handle("updateRobot", async (e, robot, oldPath) => {
  const robotPath = node_path.join(pathToRobots, robot.path);
  const robotFilePath = node_path.join(robotPath, "robot.json");
  if (oldPath) {
    const oldRobotFolderPath = node_path.join(pathToRobots, oldPath);
    if (!require$$2.existsSync(oldRobotFolderPath)) throw new Error("Cant find oldPath " + oldPath);
    require$$2.rmdirSync(oldRobotFolderPath, { recursive: true });
    require$$2.mkdirSync(robotPath);
    require$$2.writeFileSync(robotFilePath, JSON.stringify(robot));
    const updatedRobot2 = require$$2.readFileSync(robotFilePath, "utf8");
    return updatedRobot2;
  }
  if (!require$$2.existsSync(robotFilePath)) throw new Error("Can't find robot at " + robot.path);
  require$$2.writeFileSync(robotFilePath, JSON.stringify(robot, null, " "));
  const updatedRobot = JSON.parse(require$$2.readFileSync(robotFilePath, "utf8"));
  return updatedRobot;
});
electron.ipcMain.handle("deleteUserRobots", async () => {
  const robots = getRobots();
  robots.forEach((robot) => {
    require$$2.rmdirSync(node_path.join(pathToRobots, robot.path), { recursive: true });
  });
  return "deleted all user robots";
});
electron.ipcMain.handle("exportRobot", async (e, path) => {
  const win2 = getWin();
  if (!win2) throw new Error("No window available");
  const robotPath = node_path.join(pathToRobots, path);
  const robotFilePath = node_path.join(robotPath, "robot.json");
  const res = await electron.dialog.showSaveDialog(win2, {
    title: "Export Robot",
    filters: [{ name: "Robot File", extensions: ["json"] }]
  });
  if (res.canceled) return "canceled";
  const outputPath = node_path.normalize(res.filePath);
  require$$2.cpSync(robotFilePath, outputPath);
  return outputPath;
});
electron.ipcMain.handle("createPosition", async (e, path, position) => {
  console.log("Create Position", position.name);
  const robotPath = node_path.join(pathToRobots, path);
  const robotFilePath = node_path.join(robotPath, "robot.json");
  if (!require$$2.existsSync(robotFilePath)) throw new Error("Cant find robot file " + path);
  const tempFile = JSON.parse(require$$2.readFileSync(robotFilePath, "utf8"));
  tempFile.positions = tempFile.positions || [];
  tempFile.positions.push(position);
  require$$2.writeFileSync(robotFilePath, JSON.stringify(tempFile, null, " "));
  return tempFile.positions;
});
electron.ipcMain.handle("deletePosition", async (e, path, position) => {
...
electron.ipcMain.handle("updatePosition", async (e, path, position) => {
...
electron.ipcMain.handle("getPositions", async (e, path) => {
  const robotPath = node_path.join(pathToRobots, path);
  const robotFilePath = node_path.join(robotPath, "robot.json");
  const positions = JSON.parse(require$$2.readFileSync(robotFilePath, "utf8")).positions;
  return positions;
});
electron.ipcMain.handle("saveSequence", async (e, path, sequence) => {
  console.log("New Sequence", sequence.name);
  const robotPath = node_path.join(pathToRobots, path);
  const robotFilePath = node_path.join(robotPath, "robot.json");
  const tempRobot = JSON.parse(require$$2.readFileSync(robotFilePath, "utf8"));
  tempRobot.sequences = tempRobot.sequences || [];
  tempRobot.sequences.push(sequence);
  require$$2.writeFileSync(robotFilePath, JSON.stringify(tempRobot, null, " "));
  return "Saved New Sequence " + sequence.name;
});
electron.ipcMain.handle("deleteSequence", async (e, path, sequence) => {
  console.log("Delete Sequence", sequence.appId);
  const robotPath = node_path.join(pathToRobots, path);
  const robotFilePath = node_path.join(robotPath, "robot.json");
  const tempRobot = JSON.parse(require$$2.readFileSync(robotFilePath, "utf8"));
  const seqId = (tempRobot.sequences || []).findIndex((s) => s.appId === sequence.appId);
  if (seqId < 0) throw new Error("Didnt Find Sequence");
  tempRobot.sequences.splice(seqId, 1);
  require$$2.writeFileSync(robotFilePath, JSON.stringify(tempRobot, null, " "));
  return tempRobot.sequences;
});
electron.ipcMain.handle("updateSequence", async (e, path, sequence) => {
  console.log("Update Sequence", sequence.appId);
  const robotPath = node_path.join(pathToRobots, path);
  const robotFilePath = node_path.join(robotPath, "robot.json");
  const tempRobot = JSON.parse(require$$2.readFileSync(robotFilePath, "utf8"));
  const seqIdx = (tempRobot.sequences || []).findIndex((s) => s.appId === sequence.appId);
  if (seqIdx < 0) throw new Error("Didnt Find Sequence");
  tempRobot.sequences.splice(seqIdx, 1, sequence);
  require$$2.writeFileSync(robotFilePath, JSON.stringify(tempRobot, null, " "));
  return tempRobot.sequences;
});
electron.ipcMain.handle("getSequence", async (e, path, sequenceID) => {
  const robotPath = node_path.join(pathToRobots, path);
  const robotFilePath = node_path.join(robotPath, "robot.json");
  const tempSequences = JSON.parse(require$$2.readFileSync(robotFilePath, "utf8")).sequences;
  const seqIdx = tempSequences.findIndex((s) => s.appId === sequenceID);
  if (seqIdx < 0) throw new Error("Didnt Find Sequence");
  return tempSequences[seqIdx];
});
electron.ipcMain.handle("getServos", async (e, path) => {
  console.log("Get Servos Sequence", path);
  const robotPath = node_path.join(pathToRobots, path);
  const robotFilePath = node_path.join(robotPath, "robot.json");
  const bot = JSON.parse(require$$2.readFileSync(robotFilePath, "utf8"));
  return bot.servos;
});
...
if (process.platform === "win32") {
  ...
  if (squirrelEvent) {
    const appFolder = node_path.join(process.execPath, "..");
    const updateExe = node_path.join(appFolder, "..", "Update.exe");
    const exeName = node_path.join(appFolder, "MTM.exe");
    const runUpdate = (args, done) => {
      node_child_process.spawn(updateExe, args, { detached: true }).on("close", done);
    };
    switch (squirrelEvent) {
      case "--squirrel-install":
      case "--squirrel-updated":
        runUpdate(["--createShortcut", exeName, "--shortcut-locations", "StartMenu,Desktop"], () => electron.app.quit());
        break;
      case "--squirrel-uninstall":
        runUpdate(["--removeShortcut", exeName], () => electron.app.quit());
        break;
...
if (!gotTheLock) electron.app.quit();
...
function createWindow() {
  win = new electron.BrowserWindow({
    ...
    icon: node_path.join(__dirname, "/favicon.ico"),
...
electron.app.on("ready", () => {
  electron.protocol.registerFileProtocol("sound", (request, callback) => {
    ...
    const soundPath = electron.app.isPackaged ? node_path.join(__dirname, "../dist/sounds", urlPath) : node_path.join(__dirname, "../../public/sounds", urlPath);
    console.log("SOUND PATH ->", soundPath);
    callback({ path: soundPath });
  });
  electron.protocol.registerFileProtocol("img", (request, callback) => {
    const urlPath = request.url.substr(5);
    console.log("IMAGE URL ->", urlPath);
    const imgPath = electron.app.isPackaged ? node_path.join(__dirname, "../dist/images", urlPath) : node_path.join(__dirname, "../../public/images", urlPath);
    console.log("IMAGE PATH ->", imgPath);
    callback({ path: imgPath });
  });
  electron.ipcMain.on("reactIsReady", () => {
    win?.webContents.send("app_version", electron.app.getVersion());
    if (firstReactInit === true) {
      firstReactInit = false;
      console.log("React Is Ready");
      compareToLatest();
      const isInstalled = electron.app.isPackaged && !process.execPath.includes("\\out\\");
      if (isInstalled) {
        ...
        electron.ipcMain.on("installUpdate", () => {
